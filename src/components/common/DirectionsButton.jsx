...
export default function DirectionsButton({
  ...
  return (
    <a
      ...
      aria-label="Get Showroom Directions on Google Maps"
    ...
    >
      {showIcon && <MapPin className={size === 'lg' ? 'w-5 h-5' : size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4 text-[#C5A059]'} />}
      {!iconOnly && <span>{children || 'Get Directions'}</span>}
    </a>
  );
}
